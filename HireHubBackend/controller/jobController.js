...
export const getAllJob = async (req, res, next) => {
    ...
    const jobs = await JobModel.find({ deadline: false });
    res.status(200).json({
        success: true,
        jobs,
    })
...
export const postJob = async (req, res) => {
    ...
    const { title, description,categories,company, location, employmentType,salaryRangeMin,
        salaryRangeMax, requirements, responsibilities,
        postedDate, deadline, contactEmail
    } = req.body;
    ...
    const cloudinaryResponse = await cloudinary.uploader.upload(logo.tempFilePath, { folder: 'HireHub' });
    ...
    try {
        const job = await JobModel.create(
            {
                title, description,categories, company, location, employmentType, salaryRange, requirements, responsibilities,
                postedDate, deadline, contactEmail, postedBy, logo: {
...
export const countCategories=async(req,res)=>{
    ...
    // }
    try {
        const itJobs=await JobModel.find({categories:'Information Technology (IT)'});
